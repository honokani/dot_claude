...
| 0.2.10.凍結終了 | 管理境界 | ~/.claude/.gitの扱い | 削除（Phase 0.1.0 の凍結方針を終了） | 凍結維持 | 独自履歴ゼロ・working diff 39件の偽差分で保険として機能していない。最終commit `5ab47eb` は dot_claude 側にも保持済。現役リポジトリ /c/git_clone/dot_claude/.git に一本化 |
...
| 0.2.25.public化準備 | セキュリティ | 履歴側の識別子 | `git filter-repo --replace-text` で該当文字列だけ書換して force push（現 repo を維持し visibility 変更で public 化） | そのまま public 化／squash した新規 repo | ユーザー指示「履歴削除お願いします」。author メールは本人判断で残すため squash の利点（メール消去）が不要になり、履歴と commit 参照を保てる replace-text が最小変更。副作用（全ハッシュ変更・他環境の reset 必要・GitHub キャッシュ残存）は事前提示のうえ承認。作業はスクラッチ clone で実施し、ツリー差分検証後に push |
...
| 0.2.27.標準ディレクトリ | pj管理 | 作成トリガー | pj管理3ファイル読込・初期化と同時（pj管理対象プロジェクトのみ） | 初回使用時に逐次作成 | ユーザー選択。逐次作成は「_gomi が無いから退避を諦める」等の判断ブレ余地を残す。pj管理連動なら無関係の cwd（他人の repo 等）を汚さない |
| 0.2.27.標準ディレクトリ | pj管理 | git 上の扱い | 3つとも .gitignore（受領物・会話用生成物・ゴミは repo に入れない） | _gomi のみ ignore／ignore 設定しない | ユーザー選択（repo をコード専用に保つ）。追跡済ファイルの _gomi 退避は plain mv（git mv は ignore 先へ移せない）で履歴上は削除・実体は温存 |
| 0.2.27.標準ディレクトリ | pj管理 | 命名 | `_` 先頭で3つ統一（_from_owner/_deliverables/_gomi） | 接頭辞なし（from_owner 等） | ユーザー要望「名前順で先頭に」。`_` はOS・シェル・gitで特別扱いなしと確認。Jekyll等のSSGが `_` 先頭をビルド除外する規約は、非公開物ディレクトリとして望む方向に働く |
| 0.2.27.標準ディレクトリ | pj管理 | _gomiルールの配置 | 共通契約「変更管理」＋hookメッセージの2箇所 | pj管理節のみ | 削除ブロックはサブエージェントにも効くため全実行主体ルール。hookメッセージ更新でブロック発生時に正しい対処（mv → _gomi/）がその場で注入され、CLAUDE.md記憶に依存しない |
