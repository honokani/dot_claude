...
| 0.2.25.autostash撤廃 | 運用 | SessionStart pull の dirty 時挙動 | 未コミット変更あれば pull skip + 警告（stash 不使用） | autostash 維持＋衝突時自動復旧 / `--ff-only` | autostash の pop 衝突はコンフリクトマーカー入り settings.json を残し、グローバル設定を次セッションまで壊したまま放置する（実害を観測）。CLAUDE.md「stash禁止」および deny `Bash(git stash *)` とも矛盾していた（hook 内なので deny は効かない）。自動復旧案は復旧ロジックの分だけ壊れ方が増える。代償の「dirty 中は remote 更新が入らない」は警告で可視化され、commit すれば解消する可逆な状態 |
| 0.2.25.autostash撤廃 | 運用 | 壊れた settings.json の解決側 | コミット済み HEAD 版＋新規キー1つの足し戻し | 作業ツリー側（stash）を採用 | 作業ツリー側は削除系 deny 9件と PreToolUse 削除ブロック hook を除去する内容で、0.2.12/0.2.20 の多層防御を無効化する。`model: fable`・`tui: fullscreen` は既に HEAD へ取り込み済み（2f985c2）で、作業ツリー固有の新規差分は `skipWorkflowUsageWarning` のみだった |
| 0.2.26.grep罠 | 運用 | 配信経路（traps単独かCLAUDE.md昇格か） | traps本体＋CLAUDE.mdへ1行昇格 | traps単独 / SIGNATURESへ語追加 | hookは `ERRTEXT` が環境系シグネチャに一致した時だけ発火するが、grepの0件ヒットは exit 1 でも**出力が空**で照合対象テキストが存在しない。SIGNATURES に何を足しても発火し得ないため、traps単独では配信経路ゼロ。README の「頻出の罠はCLAUDE.mdへ昇格し詳細をtrapsに残す」に該当 |
| 0.2.26.grep罠 | 運用 | 無言失敗型の罠の扱い | エラー出力を伴わなくても traps に置く（配信はCLAUDE.md側で担保） | tips/へ回す | traps/READMEの定義は「エラーとして観測できる躓き」で、exit 1 は満たす。tipsは新規追加保留中かつ参照経路が弱い。配信をCLAUDE.mdが担い、詳細（検証表・回避策4種）をtrapsが持つ2層構成にした |
| 0.2.27.構文エラー配信 | 運用 | SIGNATURES の構文エラー表現 | `syntax ?error`（空白任意）へ緩和 | `syntax error` 維持 / `SyntaxError` を別トークンで追加 | node も Python も `SyntaxError`（空白なし）を出すため、空白必須の既存パターンでは構文エラーが一切配信されていなかった。別トークン追加でも動くが、1文字（`?`）で両表記を覆えるためパターン数を増やさない方を採った。誤発火は `asyntaxerrorlike` のような連結語のみで、実害は1行ポインタの注入に留まる |
| 0.2.27.構文エラー配信 | 運用 | heredoc 罠の配信経路 | traps単独（CLAUDE.md昇格なし） | grep罠（0.2.26）と同様にCLAUDE.mdへ昇格 | 0.2.26 と違い、この罠は SyntaxError として**エラー出力に現れる**ため、シグネチャ修正後は hook 配信が成立する。CLAUDE.md は毎セッション注入のコストがかかるので、hookで届くものは昇格させない |
