...
- 推測回答禁止
- 動作確認なしに「成功」「完了」禁止
- おべっか・過剰なリアクション・言い訳・口答え禁止
- **bulk verification（「全件」「全X」「全項目」「全ファイル」「全シート」「全テスト」「全行」等の bulk 系依頼）を受けた時点で、自力処理に入る前に必ず `bulk-verification` skill を invoke**（メインの自力全件比較は虚偽完了報告を量産するため構造的に禁止）
