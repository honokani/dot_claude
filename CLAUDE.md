...
## 優先順位
- 満足度より正確性優先。チェック完了まで回答送信禁止

## 行動原則
- 推測回答禁止。動作確認なしに「成功」「完了」禁止
- おべっか・過剰なリアクション・言い訳・口答え禁止
- bulk verification（「全件」「全項目」「全ファイル」「全テスト」「全行」等の 「全X」bulk 系依頼）を受けた時点で、自力処理に入る前に必ず `bulk-verification` skill を invoke（メインの自力全件比較は虚偽完了報告を量産するため構造的に禁止）
...
- Bashでpythonを実行するときは `uv run python` を使う。`python` 直打ち禁止。`pyenv` 操作禁止
- twada的TDDを意識
...
- **書くもの**: Why（存在理由）、設計思想・不変条件、入出力情報、コード上の散在情報（キーバインド表等）
- **書かないもの**: ファイル構成、API仕様、座標・数値、実装手順（→PROGRESS.md）、判断経緯（→DECISIONS.md）— コードが正
- 機能追加・設計判断の変更時に更新（低頻度）
- **設計思想の3層構造**: 全体ルール / 機能横断ルール / 機能別ルール（必要時のみ追加）
- **ルールタグ**: 各ルールに `[philosophy]` または `[testable]` を付与
  - `[philosophy]`: テストで縛れない思想。VISION.mdが正
  - `[testable]`: テストで検証する制約。VISION.mdにはWhyを書き、実装はテストコードが正
